...
def bundle_adjust_points(images, tracks, K,
                         ...
                         outlier_thresh=10.0):
    """
    Simple bundle adjustment that optimizes ONLY 3D point positions.

    - Camera intrinsics K and camera poses (R, t) are kept fixed.
    - Each residual is a 2D reprojection error (u, v) for one observation.
    - We precompute a fixed list of observations so the residual length
      is constant across calls (required by scipy.least_squares).
    """
    # 1) Which cameras are actually registered?
    registered_ids = {i for i, img in enumerate(images) if img.R is not None}

    # 2) Select tracks that:
    #    - already have a 3D point
    #    - have at least min_obs_per_track observations in registered images
    active_track_indices = []
    for ti, tr in enumerate(tracks):
        if tr.point3d is None:
            continue

        obs_reg = [(iid, kp_idx) for (iid, kp_idx) in tr.observations
                   if iid in registered_ids]
        if len(obs_reg) >= min_obs_per_track:
            active_track_indices.append(ti)

    if not active_track_indices:
        print("[BA] No tracks eligible for bundle adjustment.")
        return

    print(f"[BA] Optimizing {len(active_track_indices)} points "
          f"(min_obs_per_track={min_obs_per_track})")

    # 3) Precompute a fixed observation list:
    #    each entry = (local_point_index, image_id, keypoint_index)
    obs_list = []
    for local_idx, ti in enumerate(active_track_indices):
        tr = tracks[ti]
        for (iid, kp_idx) in tr.observations:
            if iid not in registered_ids:
                continue
            img = images[iid]
            if img.R is None or img.t is None:
                continue
            obs_list.append((local_idx, iid, kp_idx))

    if not obs_list:
        print("[BA] No valid observations for bundle adjustment.")
        return
    ...
    # 7) Write optimized points back and compute per-track error
    optimized = result.x
    removed_outliers = 0

    for local_idx, ti in enumerate(active_track_indices):
        X = optimized[3 * local_idx:3 * local_idx + 3]
        tracks[ti].point3d = X

        # Compute mean reprojection error across all registered observations
        errs = []
        tr = tracks[ti]
        for (iid, kp_idx) in tr.observations:
            if iid not in registered_ids:
        ...
        # Drop very bad points as outliers
        if mean_err > outlier_thresh or not np.isfinite(mean_err):
            tr.point3d = None
            removed_outliers += 1
